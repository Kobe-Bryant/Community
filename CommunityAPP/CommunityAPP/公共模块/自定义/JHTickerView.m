...
- (void)dealloc
{
    NSLog(@"JHTickerView(走马灯) dealloc=========");
	[tickerLabel release];
...
//	[self.layer setBorderColor:[UIColor blackColor].CGColor];  //设置背景边框
    [self.layer setBorderColor:[UIColor clearColor].CGColor];
	[self setClipsToBounds:YES];
	
	// Set the font
	tickerFont = [UIFont systemFontOfSize:17]; //[UIFont fontWithName:@"Marker Felt" size:14];
	
	// Add the label (i'm gonna center it on the view - please feel free to do your own thing)
	tickerLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, self.frame.size.height / 4-3, self.frame.size.width, self.frame.size.height)];
	[tickerLabel setBackgroundColor:[UIColor clearColor]];
	[tickerLabel setNumberOfLines:1];
    [tickerLabel setTextColor:RGB(40, 40, 43)];
...
-(void)tickerMoveAnimationDidStop:(NSString *)animationID
